function [b_scan] = recs_Tabea(i,b_scan,Offset,Chirp,ChirpN,Dispersion,SE,infa,infb,interp_par,BtECS,FileName,File,Path,Apo,complex,spectrometer,half,apo)
% by Hinnerk Schulz-Hildebrandt
% - added parallel computing support by Michael Münter
b_scan = single(reshape(b_scan,[2048 length(b_scan)/2048]))*(BtECS)-Offset*(BtECS);
...
b_scan = ifft(b_scan,SE,1); % kpace-Reconstruction

b_scan = reshape(b_scan,[2048 500 250]);
