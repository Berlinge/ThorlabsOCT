...
%%%%%% Tabeas Spezialanpassung 
% index = contains(string(List(:,1)),"ApodizationSpectrum"); % Extract Offset
% fseek(fileID,List{index,2},'bof');
% apo = single((typecast(fread(fileID,List{index,3},'uint8=>uint8'),'single')));
% id = fopen(strcat(inf.path,'\OffsetErrors.data'),'r');
% if id == -1
%   error('Cannot open file: %s', FileName);
% end
% Offset = fread(id, 2048*4, 'single');
% fclose(id);
%%%%%%
...
try
    ...
    disp('Something is wrong with Probe.txt within the OCT file');
end
...
try % Set dispersion
    ...
    try
        disp(['Dispersion load failed or wrong input, tried to load',Dispersion_Used])
        disp(inf.Probe)
        return
    catch
        prompt = {'Enter Disperspion manual:'};
        dlgtitle = 'Dispersion Input';
        dims = [1 35];
        definput = {'0'};
        answer = inputdlg(prompt,dlgtitle,dims,definput);
        inf.Dispersion = str2num(answer{1,1});
    end
end
...
try
    for i=1:size(List,1)
        ...
        if contains(File_name,'Spectral') && k<=inf.SizeY*inf.SaSA
            ...
            if strcmp(inf.rec,'1')
                %rec1(k,bscan,Offset,Chirp,ChirpN,inf.Dispersion,SE,inf.b_a,inf.b_b,inf.interp_par,BtECS,File_name,File,Path,Apo,inf.process_complex,spectrometer,inf.proc.half); % Square size determined by idx
            ...
            elseif strcmp(inf.rec,'2')
                recs_Tabea(k,bscan,Offset,Chirp,ChirpN,inf.Dispersion,SE,inf.b_a,inf.b_b,inf.interp_par,BtECS,File_name,File,Path,Apo,inf.process_complex,spectrometer,inf.proc.half,apo); % Square size determined by idx
